...
class ExpenseRepository {
  ...
  async updateTagInExpense(id, oldTag, newTag) {
    // try {
    //   const db = getDB();
    //   console.log("id", id + "oldTag",oldTag + "newTag",newTag);
    // const result = await db.collection(this.collectionName).updateOne({_id: new ObjectId(id)},{ $pull: {tags:oldTag}, $push:{tags: newTag}});
    // return result;
    // } catch (error) {      
    ...
    try {
      const db = getDB();
      const result = await db.collection(this.collectionName).updateOne({_id: new ObjectId(id), tags: oldTag}, {$set: {"tags.$": newTag}});
      return result;
    } catch (error) {      
      console.log("error", error);
      throw error;
  ...
  async deleteTagFromExpense(id, tag) {
    // const db = getDB();
    // const result = await db.collection(this.collectionName).updateOne({ _id: new ObjectId(id) },
    //   { $pull: { tags: tag } });
    // return result;
    try {
      const db = getDB(); 
